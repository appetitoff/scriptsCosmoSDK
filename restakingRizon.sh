...
RED_COLOR='\033[0;31m'
WITHOU_COLOR='\033[0m'
DELEGATOR_ADDRESS='rizon17qltem80lz3gef4a5xgnr9k2kzg8u56axu5q69'
VALIDATOR_ADDRESS='rizonvaloper17qltem80lz3gef4a5xgnr9k2kzg8u56a4ckpc4'
PWD='YOUR_PASSWORD'
...
for (( ;; )); do
        ...
        echo -e "${PWD}\ny\n" | rizond tx distribution withdraw-all-rewards --from ${ACC_NAME} --fees 20uatolo --chain-id titan-1 --yes
        ...
        echo -e "${PWD}\n${PWD}\n" | rizond tx distribution withdraw-rewards ${VALIDATOR_ADDRESS} --chain-id titan-1 --from ${ACC_NAME} --node ${NODE} --commission -y --fees 1000uatolo
        for (( timer=10; timer>0; timer-- ))
        do
        ...
        echo -e "${PWD}\n${PWD}\n" | rizond tx staking delegate ${VALIDATOR_ADDRESS} ${BAL}uatolo --chain-id titan-1 --from ${ACC_NAME} --node ${NODE} -y --fees 1000uatolo
